...
title: TypeScrip装饰器
abbrlink: 5f5fe4d5
date: 2025-08-17 16:51:08
img: /static/21.webp
categories: HTML/CSS/JS
tags:
  - typeScript
---
### **<font color='red'>一、简介</font>**
...
### **<font color='red'>二、类装饰器</font>**

#### **<font color='#10c300'>2.1、基本语法</font>**
...
#### **<font color='#10c300'>2.2、应用举例</font>**
...
const p1 = new Person("小明", 15);
console.log(p1.toString()); // [object Object]  使用装饰器后：'{"name":"小明","age":15}'


interface Person {
    a: any;
}
// 当封闭其原型对象 此行会报错：Cannot add property a, object is not extensible
Person.prototype.a = 100;
console.log(p1.a)
```
<br>

#### **<font color='#10c300'>2.3、关于返回值</font>**
...
#### **<font color='#10c300'>2.4、关于class构造类型</font>**
...
#### **<font color='#10c300'>2.5、替换被装饰的类</font>**
...
function LogTime<T extends Constructor>(target: T) {
    return class extends target { // return一个类并且这个类继承了target传来的类
        createdTime: Date;
        constructor(...args: any[]) { // 构造器可以接受任意数量/类型的参数
            super(...args); // 继承传递的任意参数
...
### **<font color='red'>三、装饰器工厂</font>**

**装饰器工厂是一个返回装饰器函数的函数（装饰器工厂中会包含一个装饰器）**，可以为装饰器添加参数，可以更灵活地控制装饰器的行为。 

**需求：**定义一个`LogInfo`类装饰器工厂，实现`Person`实例可以调用到`introduce`方法，且`introduce`中输出内容的次数，由`LogInfo`接收的参数决定。

```typescript
interface Person { // 因为给原型上添加了个方法，因此要加个类型
    introduce: () => void
}

// 定义一个装饰器工厂 LogInfo，它接受一个参数 n，返回一个类装饰器
function LogInfo(n: number) {
    return function (target: Function) { // 装饰器函数，target 是被装饰的类
...
### **<font color='red'>四、装饰器组合</font>**

> **装饰器组合**即装饰器工厂和装饰器的组合

#### **<font color='#10c300'>4.1、执行顺序</font>**

装饰器可以组合使用，执行顺序为：先【**由上到下**】的执行所有的装饰器工厂，依次获取到装饰器，然后再【**由下到上**】执行所有的装饰器。**（先执行装饰器工厂后执行装饰器，装饰器工厂由上往下，装饰器由下往上执行）**
...
console.log(p1.toString()) // {"name":"张三","age":18,"createdTime":"2024-11-22T02:49:25.234Z"}
p1.introduce() // 打印3次：我的名字：张三，我的年龄：18
console.log(p1.getTime()) // 该对象创建时间为：Fri Nov 22 2024 10:50:19 GMT+0800 (中国标准时间)
```

<br>

### **<font color='red'>五、属性装饰器</font>**
#### **<font color='#10c300'>基本语法</font>**
```typescript
/* 
  参数说明：
...
#### **<font color='#10c300'>关于属性遮蔽</font>**

> 常规逻辑：给实例对象this添加一个age属性，js会先查看是否有这个属性，没有就查看原型对象上是否有。如下代码在给原型对象添加age属性之前就已经new了一个实例对象，因此实例age是18 原型对象的age是130
...
console.log(P1);  // {name: '小明', age: 18}  原型对象上age是130
 
```

![image-20241125164721617](https://picgo-2024.oss-cn-beijing.aliyuncs.com/img/202411251647729.png)



> **属性遮蔽：**如下代码中：当构造器中的`this.age = age`试图在实例上赋值时，实际上是调用了原型上`age`属性的`set`方法。
>
...
console.log(p1)
```
![image-20241125170107136](https://picgo-2024.oss-cn-beijing.aliyuncs.com/img/202411251701230.png)

<br>

#### **<font color='#10c300'>应用举例</font>**
